...
```
/endpoint?orderBy=name&sortedBy=desc
```
...
```
/endpoint?search=first keyword;second keyword
```

#### Search in specific field:
```
/endpoint?search=field:keyword here
```

#### Search in specific fields for multiple keywords: 
```
/endpoint?search=field1:first field keyword;field2:second field keyword
```

#### Define query condition:

```
/endpoint?search=field:keyword&searchFields=name:like
```
...
```
/endpoint?search=field1:first keyword;field2:second keyword&searchFields=field1:like;field2:=;
```
...
```shell
  ...
  "meta": {
    "pagination": {
      ...
      "total_pages": 1111,
      "links": {
        "previous": "http:///endpoint?page=21"
      }
    }
  }
```
...
```shell
curl -X POST -H "Accept: application/json" -H "Content-Type: application/x-www-form-urlencoded; -F "email=admin@admin.com" -F "password=admin" -F "=" "http:///v2/register"
```

Calling protected endpoint (passing Bearer Token) example:

```shell
curl -X GET -H "Accept: application/json" -H "Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..." -H "http:///v1/users"
```
